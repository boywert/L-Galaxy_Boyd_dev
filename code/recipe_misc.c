...
void get_stellar_disk_radius(int p)
{
  ...
  /* if(dstar != dstar) { */
  /*   if(Gal[p].Type == 0) */
  /*     printf("StellarDiskRadius = %0.8f StellarSpin %f %f %f Vmax %f\n",dstar,Gal[p].StellarSpin[0],Gal[p].StellarSpin[1],Gal[p].StellarSpin[2],Gal[p].Vmax); */
  /*   else */
...
void init_galaxy(int p, int halonr)
{
  ...
  // printf("vmax = %lg, mass Vvir %f Mvir %f Rvir %f\n",Gal[p].VmaxGal[p].Vvir, Gal[p].Mvir, Gal[p].Rvir); 
  ...
  for(outputbin = 0; outputbin < NOUT; outputbin++)
    {
      ...
      /* printf ("obin %d time %lg mass = %lg X1 = %lg, factor = %lg\n",outputbin,time,(double)0.1*Gal[p].HaloM_Crit200,X1,(fmet1 * (f1 * NPhotTables[metindex][tabindex] + */
      /* 				    f2 * NPhotTables[metindex][tabindex + 1]) + */
      /* 			   fmet2 * (f1 * NPhotTables[metindex + 1][tabindex] + */
      /* 				    f2 * NPhotTables[metindex + 1][tabindex + 1]))); */
      if(PhotonsToAdd != PhotonsToAdd)
	printf("Rank: x1 = %lg photon = %lg f1 = %lg fmet1 = %lg gammatab = %lg\n",ThisTask,X1,PhotonsToAdd,f1,fmet1,NPhotTables[metindex][tabindex]);
